...
| 2026-09-03 | Enabled `schedule:` cron automation via a new, dedicated workflow (`.github/workflows/daily-pipeline.yml`), and decided the Vercel deployment shape (git-integrated auto-deploy, `npm run build` only). | See ADR 0004 for full context, including the deep-research finding that the reference project has no transferable CI/cron playbook to copy. Vercel connection itself is decided but not yet executed — blocked on reconnecting the Vercel MCP integration. | @sairamugge |
