...
- **Phase 9: cron automation shipped, Vercel decided but not yet connected.** Real, live fetch from Hacker News' free Algolia API and arXiv's Atom API; real LLM scoring via AWS Bedrock (`src/lib/llmCuration.ts`, Sonnet 5→Sonnet 4.6→Opus→Haiku fallback chain, Sonnet 5 leading as of ADR 0005) as the default path, with the original deterministic placeholder retained as an explicit no-credentials fallback — see `decisions.md` ADR 0003; an Astro static site rendering committed digests via Content Collections; one Chart.js bar-chart island; a public `/stats` cost page. `.github/workflows/daily-pipeline.yml` now runs the pipeline daily via a real `schedule:` trigger and commits the result itself — see ADR 0004. Real Vercel deployment is decided (git-integrated auto-deploy, `npm run build` only) but not yet executed, pending the Vercel MCP integration's reconnection.
...
- **Curation is now a real LLM call via AWS Bedrock, with the honest placeholder kept as an explicit fallback.** `src/lib/llmCuration.ts` scores every fetched item (HN + arXiv) in one forced-tool-use batched call per pipeline run, using a credential shared with the sibling Anvilry project. `src/lib/curation.ts`'s deterministic placeholder (computed from real, already-fetched fields — `points`/`num_comments`/`title` for HN, recency for arXiv) still exists and is used, with a loud console warning, whenever credentials aren't configured (e.g. local dev) or the LLM's response omits a specific item. See `decisions.md`/ADR 0003 for full rationale and `SOUL.md`'s non-negotiables for why the fallback is never dressed up as real judgment.
...
- **Vercel deployment shape decided (git-integrated, `npm run build` only, never the pipeline) but not yet connected** — the Vercel MCP integration's token needs reconnecting before `create_git_project` can run.
...
4. **Enable real Vercel deployment.** Shape decided (ADR 0004); execution blocked on reconnecting the Vercel MCP integration.
