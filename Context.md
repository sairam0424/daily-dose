...
- **Archive index (`/archive/`) is a flat, ungrouped list — confirmed fine at current scale and well beyond it.** A `/deep-research` pass checked tdd.cat's own live `/archive/` page (the direct feature-parity comparable) directly: still flat/ungrouped at 47 entries, no grouping/search/pagination. TLDR.tech is the same at ~20 entries. Only decade-plus archives (Stratechery, Daring Fireball) add year/month headings, and even those never use collapsible `<details>`/search. Revisit only once real entry count exceeds ~365 (add plain `<h2>` year headings; month sub-headings only past ~60-80 entries/year). Do not add a search library (e.g. Pagefind) until browser Ctrl+F genuinely stops being sufficient — no new dependency is justified at today's or the near-future scale.
