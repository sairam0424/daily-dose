...
4. **Real data over fabricated data.** The fetch step is live against HN's real, public Algolia API, every run, with no hardcoded fallback content. A fabricated "example story" to make the site look populated is a lie to the reader even if no one ever notices.
5. **Small and finishable over comprehensive.** A one-source (HN-only) walking skeleton that actually runs, fetches real data, and renders beats a two-source (HN + arXiv) plan that doesn't yet work. Matches `../Not-Humans-Lab/SOUL.md`'s "small and finishable over big and impressive."
