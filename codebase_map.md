...
│   │   │                               forced-tool-use batched call/run, Sonnet→Opus→Haiku
...
| `src/lib/llmCuration.ts` | The real AWS Bedrock LLM scoring client (`AnthropicBedrock`). `scoreItemsWithLLM()` scores a whole batch in one forced-tool-use call, Sonnet→Opus→Haiku fallback on `NotFoundError`/`BadRequestError`, Zod-validated response (`ScoresResponseSchema`). This is the default scoring path as of ADR 0003. | Imported by `scripts/pipeline.ts`; fully mocked in `tests/llmCuration.test.ts`. See `CLAUDE.md`'s plan-mode gate before editing. |
