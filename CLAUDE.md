...
- HN, arXiv, GitHub (ADR 0006), and Dev.to (ADR 0007) sourcing, real LLM curation (ADR 0003), static-site rendering, the `/stats` cost-transparency page, `schedule:` cron automation, and real Vercel deployment (ADR 0004, live at https://daily-dose-hazel-delta.vercel.app) are all real and shipped — every item on `Context.md`'s original roadmap is done. Any further scope (a fifth source, richer stats, alerting) is a new decision, not a resumption of a deferred one.
